...
#define VKMAP_INSERT(x) vkMap.emplace(std::make_pair((#x), x))
...
::byte StrToVK::Convert(const std::string& str)
{
	if (str.length() == 1) // ASCII
	{
		const char& firstLetter = toupper(str[0]);
		if ((firstLetter >= ('0') && firstLetter <= ('9')) ||
			(firstLetter >= ('A') && firstLetter <= ('Z'))) return (::byte)firstLetter;
...
::byte StrToVK::GetVK(const std::string& str)
{
	return GetInstance().Convert(str);
}
